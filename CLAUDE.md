...
Found during verification and deliberately left; neither is a regression.

1. **Read state is only ever written for messages the client has loaded.** The effect in `ChatWindow.jsx:58` marks messages from the `messages` array, which holds one page (30) for the active chat. Anything older that was never scrolled into view stays unread in Mongo forever. Invisible until `unreadCount` started coming from the server; now it shows as a badge that opening the chat cannot clear. Real example: `emre` has 52 unread sitting at positions 166–368 of a 661-message group. Fixing it means marking the whole chat read on open (a `markChatAsRead` event) rather than marking a page at a time — which also decides whether senders get read receipts for messages the reader never actually looked at.
